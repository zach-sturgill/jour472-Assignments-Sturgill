...
**A1. The leading word is "failing" and the top 10 words likely describe how somebody failed to do something resulting in the sanction.**

```{r}
sanctions <- read_csv("~/Documents/GitHub/jour472-Assignments-Sturgill/labs/lab_12/data/md_attorney_sanctions.csv")
...
**A2. The top phrase is attorney trust account appearing 393 times. In legal terms, it is when an attorney manages funds for clients.**
...
**A3. Sanctions related to attorney trust accounts have increased over time and interestingly hit a peak during COVID years. To clarify the most important reasons attorneys get sanctioned, you could group common phrases (trust account is mentioned 3 times in the top 10 from Q2) and compare the trends over the years of all of the top phrases.**

```{r}
q3 <- sanctions |>
  filter(str_detect(text, "attorney trust account")) |>
  group_by(fiscal_year) |>
  summarize(count_phrase = n()) |>
  arrange(desc(count_phrase))
```
