...
In the space below, write the data type for the "id" column and the "seats" column (it's in light gray, offset by \< \>). **Answer: "id" is chr and "seats" is dbl**
...
umd_courses |>
  ...
  summarise(
    ...
    mean_seats = mean(seats),
    median_seats = median(seats),
    min_seats = min(seats), # add minimum seats here
    max_seats = max(seats) # add maximum seats here
  ) |>
  arrange(desc(total_seats))
```
...
1.  What department has the most classes?

2.  What course id has the most sections?

3.  How many courses have NA as their instructor?
